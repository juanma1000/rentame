...
# Use the app's own settings (env vars) for the connection URL instead of the
# static value in alembic.ini, so migrations always target the same database
# as the running app.
config.set_main_option("sqlalchemy.url", get_settings().database_url)

# `Base.metadata` is the single source of truth for autogenerate, shared by
# every domain's persistence models (see shared/infrastructure/database.py).
