...
- [ ] 10.1 Crear el feature branch `feature/hu-002-inmuebles-app` desde main
- [ ] 10.2 Verificar la creación del branch y el estado del branch actual

## 11. Frontend: Selector de Propietario (TDD)

- [ ] 11.1 Test: `PublicarInmueblePage` muestra el selector de propietario solo cuando la sesión es de rol `agente` (usando `useAuth()`)
- [ ] 11.2 Test: el selector se llena con los propietarios de `GET /agencias/mia/propietarios` con relación `activa`
- [ ] 11.3 Test: el envío incluye el `propietario_id` seleccionado en el request a `publicarInmueble`
- [ ] 11.4 Test: `EditarInmueblePage` no requiere selector (edita el inmueble existente, propietario ya fijo) pero sí debe permitir que un agente distinto al publicador la use
- [ ] 11.5 Implementar el selector condicional por rol en `PublicarInmueblePage.tsx`, y el servicio `agencias.api.ts` (nuevo) o extensión de `inmuebles.api.ts` para consumir `GET /agencias/mia/propietarios`
- [ ] 11.6 Confirmar que todos los tests de la sección pasan (Green)

## 12. Frontend: Panel "Inmuebles que gestiono" (TDD)

- [ ] 12.1 Test: renderiza la lista de inmuebles gestionados con badge de estado, análogo a `MisInmueblesPage`
- [ ] 12.2 Test: muestra estado vacío si el agente no gestiona ningún inmueble
- [ ] 12.3 Implementar `InmueblesGestionadosPage.tsx` consumiendo `GET /inmuebles/gestionados`, reusando componentes de `MisInmueblesPage` donde tenga sentido sin sobre-abstraer
- [ ] 12.4 Confirmar que todos los tests de la sección pasan (Green)

## 13. Revisar y Actualizar Unit Tests Existentes — Frontend (OBLIGATORIO)

- [ ] 13.1 Correr la suite completa de Jest/RTL de `inmuebles-app`, `shell` y `@rentame/auth`
- [ ] 13.2 Confirmar que no hay regresiones sobre los tests existentes de HU-001
