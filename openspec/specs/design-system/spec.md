...
Sistema de diseño compartido (`@rentame/design-tokens`, `@rentame/ui`) que define tokens visuales (tipografía, spacing, radios, sombras, transiciones, z-index, breakpoints, color) y componentes reutilizables (Button, Badge, PropertyCard, navegación) con identidad Navy/dorado para las aplicaciones de Rentame, evitando estilos ad-hoc y badges o funcionalidades que no estén respaldadas por datos reales de backend.
